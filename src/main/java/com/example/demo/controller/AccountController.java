...
@Tag(name = "Account Management", description = "APIs for managing account data")
...
public class AccountController {
    ...
    @Operation(summary = "Process accounts sequentially", description = "Process all account records sequentially")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Sequential processing completed"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    @PostMapping("/sequential-processing")
    public ResponseEntity<List<AccountResponseDto>> processAccountsSequentially() {
        List<AccountResponseDto> response = accountService.processAccountRecordsSequentially();
        return ResponseEntity.ok(response);
    }

    @Operation(summary = "Update account balance with interest", description = "Update account balance with interest and reset cycle amounts")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Balance updated successfully"),
        @ApiResponse(responseCode = "404", description = "Account not found"),
        @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    @PostMapping("/{id}/update-balance-interest")
    public ResponseEntity<Void> updateAccountBalanceWithInterest(@PathVariable Long id, 
                                                                 @RequestParam java.math.BigDecimal interest) {
        accountService.updateAccountBalanceWithInterest(id, interest);
