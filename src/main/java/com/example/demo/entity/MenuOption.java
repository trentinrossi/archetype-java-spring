...
public class MenuOption {
    ...
    public void validate() {
        if (optionNumber == null || optionNumber < 1) {
            throw new IllegalArgumentException("Option number must be greater than 0");
        }
        ...
        if (requiredUserType == null || (!requiredUserType.equals("A") && !requiredUserType.equals("U"))) {
            throw new IllegalArgumentException("Required user type must be A (admin) or U (user)");
        }
    }

    public boolean isAdminOnly() {
        return "A".equals(requiredUserType);
    }
}
