...
public class CreateAccountRequestDto {
    ...
    @Schema(description = "Indicates if the account is active. Possible values: 'A' for active, 'I' for inactive", example = "A", required = true)
    ...
    @Schema(description = "Current balance of the account, includes decimal places for cents", example = "1500.50", required = true)
    @NotNull(message = "Current balance is required")
    @DecimalMin(value = "0.0", inclusive = true, message = "Current balance must be non-negative")
    private BigDecimal acctCurrBal;

    @Schema(description = "Maximum credit limit for the account, includes decimal places for cents", example = "5000.00", required = true)
    @NotNull(message = "Credit limit is required")
    @DecimalMin(value = "0.0", inclusive = true, message = "Credit limit must be non-negative")
    private BigDecimal acctCreditLimit;

    @Schema(description = "Maximum cash credit limit for the account, includes decimal places for cents", example = "1000.00", required = true)
    ...
    @Schema(description = "Date when the account was opened, format YYYY-MM-DD", example = "2023-01-15", required = true)
    @NotNull(message = "Account open date is required")
    private LocalDate acctOpenDate;

    @Schema(description = "Date when the account expires, format YYYY-MM-DD", example = "2026-01-15", required = true)
    ...
    @Schema(description = "Current cycle credit amount, includes decimal places for cents", example = "250.00", required = true)
    ...
    @Schema(description = "Current cycle debit amount, includes decimal places for cents", example = "150.00", required = true)
