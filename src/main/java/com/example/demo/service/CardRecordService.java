...
public class CardRecordService {
    ...
    public CardRecordResponseDto createCardRecord(CreateCardRecordRequestDto request) {
        ...
        if (cardRecordRepository.existsByCardNumber(request.getCardNumber())) {
            throw new IllegalArgumentException("Card record with this number already exists");
        }
        ...
        log.info("Card record created successfully with number: {}", savedCardRecord.getCardNumber());
        
        return convertToResponse(savedCardRecord);
    }

    @Transactional(readOnly = true)
    public Optional<CardRecordResponseDto> getCardRecordByNumber(String cardNumber) {
        log.info("Retrieving card record with number: {}", cardNumber);
    ...
    public CardRecordResponseDto updateCardRecord(String cardNumber, UpdateCardRecordRequestDto request) {
        ...
        log.info("Card record updated successfully with number: {}", updatedCardRecord.getCardNumber());
    ...
    public void deleteCardRecord(String cardNumber) {
        ...
        if (!cardRecordRepository.existsByCardNumber(cardNumber)) {
            throw new IllegalArgumentException("Card record not found");
        }
        
        cardRecordRepository.deleteById(cardNumber);
        log.info("Card record deleted successfully with number: {}", cardNumber);
    }
