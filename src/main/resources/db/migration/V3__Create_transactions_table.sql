...
-- Bill payment transaction records
...
CREATE TABLE transactions (
    ...
    CONSTRAINT fk_transaction_account FOREIGN KEY (account_id) REFERENCES accounts(acct_id) ON DELETE CASCADE
);
...
COMMENT ON COLUMN transactions.tran_source IS 'Source of transaction (e.g., POS TERM)';
...
COMMENT ON COLUMN transactions.account_id IS 'Account ID for the transaction';
COMMENT ON COLUMN transactions.created_at IS 'Transaction creation timestamp';
COMMENT ON COLUMN transactions.updated_at IS 'Transaction last update timestamp';
